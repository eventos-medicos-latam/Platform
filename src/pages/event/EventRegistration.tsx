...
import { publicTickets } from '../../data/tickets';
...
export function EventRegistration() {
  ...
  const [ticketId, setTicketId] = useState(params.get('ticket') ?? tickets[0]?.id ?? '');
  ...
  const amount = ticket ? withVat(ticket.price, ticket.vatRate) : null;
  ...
  const pay = () => {
    ...
    window.setTimeout(() => {
      addRegistration({
        ...
        trackInterestId: form.trackInterestId,
        modality: 'presencial',
        amount,
        paymentStatus: amount === null ? 'pending' : 'approved',
        qrCode: `HB2-${String(Math.floor(Math.random() * 9000) + 1000)}`,
        qrStatus: 'active',
        source: `inscripcion-${edition.slug}`,
        crmSynced: true,
        consentCommercial: form.consentCommercial,
          ...
          <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
                      ...
                      <div className="mt-6 flex items-center gap-4 rounded-xl border border-line bg-canvas p-5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-ink-muted">
                            Se emite al aprobarse el pago y se valida en el check-in.
                          </p>
                        </div>
                      </div>
                    </div> : null}
            ...
            <aside className="rounded-2xl border border-line bg-white p-6">
              ...
              <ul className="mt-4 space-y-2.5">
                {tickets.map((item) => {
                const final = withVat(item.price, item.vatRate);
